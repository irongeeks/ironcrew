...
| `server/ironcrew/scheduler/`                        | The background loop and the five jobs it runs by default — queue drain, routines, mailboxes, messengers, lease sweep — plus `audit-ship` when a sink is configured. No persistence of its own; it only decides when to look (docs/SERVICE.md). |
