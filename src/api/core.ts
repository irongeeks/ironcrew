...
export function writeStoredCsrfToken(token: string): void {
...
export async function postWithIdempotency<T>(
  ...
  for (let attempt = 0; attempt <= POST_RETRY_LIMIT; attempt++) {
    ...
    try {
      ...
      if (
        canRetryAuth &&
        url !== SESSION_BOOTSTRAP_PATH &&
        (r.status === 401 || (r.status === 403 && errCode === "csrf_token_invalid"))
      ) {
...
async function doBootstrapSession(promptOnUnauthorized: boolean): Promise<boolean> {
  try {
    ...
    if (response.status === 401 && promptOnUnauthorized) {
      // Don't prompt for token on remote — LoginPage handles authentication
      const authStatus = await checkAuthStatus().catch(() => null);
      if (authStatus?.isRemote && authStatus?.passwordConfigured) {
        return false;
      }
...
export async function request<T>(url: string, init?: RequestInit, canRetryAuth = true): Promise<T> {
  // For mutations, ensure a CSRF token is in place before firing. This avoids
  // a 403 race on the first save right after a hard reload, when the session
  // bootstrap may still be in flight.
  if (canRetryAuth && url !== SESSION_BOOTSTRAP_PATH && isMutationMethod(init?.method) && !readStoredCsrfToken()) {
  ...
  if (
    canRetryAuth &&
    url !== SESSION_BOOTSTRAP_PATH &&
    (r.status === 401 || (r.status === 403 && errorCode === "csrf_token_invalid"))
  ) {
    await bootstrapSession({ force: true });
...
export async function checkAuthStatus(): Promise<AuthStatus> {
  const r = await fetch(`${base}/api/auth/status`, { credentials: "same-origin" });
  if (!r.ok) return { passwordConfigured: false, authenticated: false, isRemote: false };
  return r.json();
}
