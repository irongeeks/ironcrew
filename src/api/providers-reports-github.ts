...
export async function getGitHubPulls(owner: string, repo: string): Promise<{ pulls: GitHubPull[] }> {
  return request<{ pulls: GitHubPull[] }>(
...
export async function getProjectGitStatus(projectId: string): Promise<ProjectGitStatus> {
  return request<ProjectGitStatus>(`/api/projects/${projectId}/git-status`);
}
