...
describe("api client", () => {
  ...
  it("createDepartment가 JSON body로 POST 요청을 보낸다", async () => {
    window.sessionStorage.setItem("claw_api_csrf_token", "csrf-existing");
    const fetchMock = vi.fn();
  ...
  it("bootstrapSession은 401에서 prompt 입력 토큰으로 재시도한다", async () => {
    ...
    fetchMock
      .mockResolvedValueOnce(jsonResponse({ error: "unauthorized" }, 401))
      .mockResolvedValueOnce(jsonResponse({ passwordConfigured: false, authenticated: false, isRemote: false }, 200))
      .mockResolvedValueOnce(jsonResponse({ ok: true }, 200));
    vi.stubGlobal("fetch", fetchMock as unknown as typeof fetch);
