      ...
      if (!matchedAgent) {
        window.alert(
          pickLang(normalizeLanguage(settings.language), {
            ...
            zh: "Could not find the requested agent.",
          }),
        );
        return;
      }
      ...
      try {
        if (item.kind === "agent_request") {
          ...
          const replyContent = pickLang(locale, {
            ...
            zh: `[Decision Reply] Please proceed with option ${option.number}. (${option.label})`,
          });
          await api.sendMessage({
            receiver_type: "agent",
            ...
            if (!note) {
              window.alert(
                pickLang(locale, {
                  ...
                  ja: "追加要請が空です。",
                  zh: "Additional request is empty.",
                }),
              );
              return;
            }
          ...
          if (replyResult.action === "start_project_review_blocked") {
            ...
            window.alert(
              pickLang(locale, {
                ...
                zh: `Team-lead meeting start is on hold. Resolve required gates first.${blockedSummary}`,
              }),
            );
          }
        ...
        window.alert(
          pickLang(locale, {
            ...
            zh: "Failed to send decision reply. Please try again.",
          }),
        );
        // Re-throw so callers (DecisionInboxModal) can preserve draft/selections on failure
        throw error;
      } finally {
