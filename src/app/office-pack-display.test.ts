...
describe("office pack display helpers", () => {
  ...
  it("before hydration prefers pack department labels/icons over DB metadata", () => {
    ...
    // Non-development packs only show pack-scoped departments — unrelated global departments are excluded.
    expect(output.some((dept) => dept.id === "operations")).toBe(false);
  });
  ...
  it("hides foreign-pack seed agents from merged lists in non-development packs", () => {
    ...
    // Non-development packs only show pack-scoped agents — unrelated global agents are excluded.
    expect(mergedAgents.some((agent) => agent.id === "dev-leader")).toBe(false);
  });
});
