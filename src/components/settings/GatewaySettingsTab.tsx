...
export default function GatewaySettingsTab({ t, form, setForm, persistSettings }: ChannelSettingsTabProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setSendStatus({
        ok: true,
        msg: t({ en: "Message sent", de: "Nachricht gesendet" }),
      });
      setSendText("");
    } catch (error) {
    ...
    >
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: "var(--th-text-secondary)" }}>
          {t({ en: "Channel Messaging", de: "Kanal-Messaging" })}
        </h3>
        {chatEditor.saved && (
      ...
      >
        <div className="flex items-center justify-between">
          <div className="text-sm font-semibold" style={{ color: "var(--th-text-heading)" }}>
            {t({ en: "Test Send", de: "Testnachricht senden" })}
          </div>
        ...
        <div>
          <label className="block text-xs mb-1" style={{ color: "var(--th-text-secondary)" }}>
            {t({ en: "Target Session", de: "Zielsitzung" })}
          </label>
          {chatRows.length === 0 ? (
        ...
        <textarea
          ...
          rows={3}
          placeholder={t({ en: "Type a test message...", de: "Testnachricht eingeben..." })}
        ...
        >
          {sending ? t({ en: "Sending...", de: "Wird gesendet..." }) : t({ en: "Send", de: "Senden" })}
        </button>
