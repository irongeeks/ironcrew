...
export default function ProjectManagerContent({
  agents,
  departments = [],
  onClose,
  embedded = false,
  ...
  const handleDelete = useCallback(async () => {
    ...
    const confirmed = window.confirm(
      t({
        ...
        de: `Projekt '${selectedProject.name}' löschen?`,
      }),
    );
    if (!confirmed) return;
  ...
  const sidebarBlock = (
    <div
      className={`w-full ${embedded ? "md:w-[280px] lg:w-[330px]" : "md:w-[330px]"} ${selectedProjectId || isCreating || githubImportMode ? "hidden md:block" : "block"}`}
    >
      <ProjectSidebar
  ...
  const contentBlock = (
    ...
    >
      ...
      {githubImportMode ? (
        ...
        <>
          <div className="px-5 py-3" style={{ borderBottom: "1px solid var(--th-border)" }}>
  ...
  if (embedded) {
    return (
      <div
        className="flex h-full min-h-[calc(100vh-8rem)] flex-col overflow-hidden shadow-2xl md:flex-row"
        style={{
  ...
  return (
    ...
    >
      <div
        className="flex h-[86vh] w-[min(1180px,95vw)] flex-col overflow-hidden shadow-2xl md:flex-row"
        style={{
          background: "var(--th-bg-secondary)",
          border: "1px solid var(--th-border)",
          borderRadius: "var(--radius-lg)",
