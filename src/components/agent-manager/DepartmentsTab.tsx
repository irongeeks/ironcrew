...
interface DepartmentsTabProps {
  ...
  onAssignRoom: (deptId: string, roomSlot: number | null) => void;
}
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {deptOrder.map((dept, index) => {
          ...
          const assignedRoom = roomAssignments[dept.id] ?? null;
          return (
            <div
              key={dept.id}
              draggable
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    ...
                    style={{
                      background: assignedRoom ? "rgba(59,130,246,0.15)" : "rgba(148,163,184,0.12)",
                      color: assignedRoom ? "#60a5fa" : "var(--th-text-muted)",
                    }}
                  >
                    {assignedRoom ? tr(`방 ${assignedRoom}`, `Room ${assignedRoom}`) : tr("방 자동", "Auto Room")}
                  </span>
                </div>
                {dept.description && (
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  onAssignRoom(dept.id, value ? Number(value) : null);
                }}
                ...
                title={tr("지도 방 슬롯 지정", "Assign map room slot")}
              >
                <option value="">{tr("자동", "Auto")}</option>
                {deptOrder.map((_, slotIndex) => (
                  <option key={`${dept.id}-slot-${slotIndex + 1}`} value={slotIndex + 1}>
                    {tr(`방 ${slotIndex + 1}`, `Room ${slotIndex + 1}`)}
                  </option>
                ))}
              </select>
            </div>
