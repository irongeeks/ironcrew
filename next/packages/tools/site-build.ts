...
export async function siteProject(stack: SiteStack, html: string): Promise<Record<string, string>> {
  ...
  if (stack === "react") {
    ...
    files["src/concept.mjs"] = `export default ${JSON.stringify(html)};\n`;
    files["src/app.mjs"] =
      `import {createElement,createRoot} from './vendor/react-runtime.mjs';import concept from './concept.mjs';const source=new DOMParser().parseFromString(concept,'text/html');document.title=source.title||'Website';for(const style of source.head.querySelectorAll('style'))document.head.append(style.cloneNode(true));for(const node of source.body.querySelectorAll('script,base,iframe,object,embed'))node.remove();function App(){return createElement('div',{dangerouslySetInnerHTML:{__html:source.body.innerHTML}});}createRoot(document.getElementById('root')).render(createElement(App));\n`;
    files["public/index.html"] =
      '<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Website</title></head><body><div id="root"></div><script type="module" src="./app.mjs"></script></body></html>\n';
    files["build.mjs"] =
    ...
    if (/<\?/.test(html)) throw new ToolError("php_in_concept_denied");
    ...
    files["theme/templates/index.html"] = "<!-- wp:html -->\n" + body + "\n<!-- /wp:html -->\n";
