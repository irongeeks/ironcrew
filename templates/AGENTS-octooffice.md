...
- German messasge -> reply in German
...
- If no meeting: Planning team leader directly delegates to the best agent via smart routing
...
- Routing can chain across specialized departments (Design, Knowledge) before final implementation
...
1. Analyze content -> select the appropriate CLI agent
   - **Coding tasks**: Claude Code, Codex, OpenCode, Copilot, Antigravity
   - **Design tasks**: Design department agent first, then implementation handoff
   - **Knowledge/docs tasks**: Knowledge department with docs provider routes (`/api/knowledge/docs/*`)
2. Prefer cross-agent chaining when specialized review or handoff is required:
   - design -> QA/design review -> implementation
   - docs sync -> implementation context update
3. **Check `project_path`** — if empty, ask the user before proceeding
4. **Check for existing work** — if the task has prior terminal logs, ask whether to continue or start fresh
5. Assign to agent and start execution

### 2-1. Server-Aware Routing

When task metadata indicates external runtime requirements (image generation, LLM gateway, DB, storage):

1. Use server allocation APIs (`/api/ops/servers/allocations/request`) before execution.
2. If queue status is returned, keep task in waiting state and monitor queue processing.
3. On completion/failure, release allocation (`/api/ops/servers/allocations/release`).
...
### Follow-Up Routing
When a follow-up request is submitted for a task in review, the CEO evaluates the request and routes it to the most appropriate agent/department. Agents may receive supplement rounds for work outside their original assignment if the CEO determines a different agent is better suited. Pipeline resets and new task creation are also possible outcomes — agents should check their continuation brief for updated context when receiving a supplement round.
...
# Knowledge docs providers
curl http://127.0.0.1:__PORT__/api/knowledge/docs/providers

# Server management
curl http://127.0.0.1:__PORT__/api/ops/servers
