...
export function initializeOAuthRuntime(deps: OAuthRuntimeDeps): OAuthRuntimeHelpers {
  ...
  // Add department multilingual + prompt columns
  try {
  ...
  // Backfill existing department multilingual names (empty values only)
  try {
    db.exec(`
    UPDATE departments SET name_ja = '企画チーム',              name_zh = 'Planning Team'    WHERE id = 'planning' AND (name_ja = '' OR name_ja IS NULL);
    UPDATE departments SET name_ja = '開発チーム',              name_zh = 'Development Team' WHERE id = 'dev' AND (name_ja = '' OR name_ja IS NULL);
    UPDATE departments SET name_ja = 'デザインチーム',          name_zh = 'Design Team'      WHERE id = 'design' AND (name_ja = '' OR name_ja IS NULL);
    UPDATE departments SET name_ja = '品質管理チーム',          name_zh = 'QA Team'          WHERE id = 'qa' AND (name_ja = '' OR name_ja IS NULL);
    UPDATE departments SET name_ja = 'ブラウザテストチーム',    name_zh = 'Browser Testing Team' WHERE id = 'browser' AND (name_ja = '' OR name_ja IS NULL);
    UPDATE departments SET name_ja = 'インフラセキュリティチーム', name_zh = 'DevSecOps Team'  WHERE id = 'devsecops' AND (name_ja = '' OR name_ja IS NULL);
    UPDATE departments SET name_ja = '運営チーム',              name_zh = 'Operations Team'  WHERE id = 'operations' AND (name_ja = '' OR name_ja IS NULL);
  `);
  } catch {
    /* already backfilled */
  }
  ...
  try {
    ...
    if (!hasBrowserCheck) {
      ...
      INSERT INTO agents_new SELECT id, name, name_ko, '', '', department_id, role, 0, cli_provider, oauth_account_id, NULL, NULL, NULL, NULL, avatar_emoji, NULL, personality, status, current_task_id, created_at FROM agents;
      DROP TABLE agents;
