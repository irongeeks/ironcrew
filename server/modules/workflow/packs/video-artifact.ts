...
function resolveVideoArtifactSpec(input: {
 ...
 * when named candidates are not found. Checks `video_output/`, `video_output/clips/`, and `out/`.
 */
export function discoverVideoArtifact(rootDir: string): string | null {
  const searchDirs = [VIDEO_OUTPUT_DIR, path.join(VIDEO_OUTPUT_DIR, "clips"), REMOTION_OUTPUT_DIR];
