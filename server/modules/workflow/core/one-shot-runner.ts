  ...
  async function runAgentOneShot(
    ...
    try {
      if (provider === "api") {
        ...
        const profile = provider === "openclaw" ? agent.cli_profile || undefined : undefined;
        const args = buildAgentArgs(provider, model, reasoningLevel, { noTools, profile });
        // openclaw: pass prompt via --message flag with a session-id (does not read stdin)
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          // openclaw uses --message flag; other providers use stdin
          if (provider !== "openclaw") {
