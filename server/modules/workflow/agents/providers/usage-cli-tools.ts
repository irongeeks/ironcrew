...
export function createUsageCliTools(deps: CreateUsageCliToolsDeps) {
  ...
  const CLI_TOOLS: CliToolDef[] = [
    {
      ...
      checkAuth: () => {
        ...
        // Claude Code stores OAuth tokens in .credentials.json
        if (jsonHasKey(path.join(home, ".claude", ".credentials.json"), "claudeAiOauth")) return true;
        return false;
      },
    },
    {
    ...
    {
      name: "openclaw",
      authHint: "Profile-based — configure via: openclaw --profile <name> config set ...",
      checkAuth: () => {
        // openclaw uses per-profile config; consider authenticated if the binary exists
        // (actual model availability is validated at spawn time)
        return true;
      },
    },
  ];
  ...
  function checkTokenFreshness(provider: string): "valid" | "expired" | "unknown" {
    switch (provider) {
      case "claude":
        return readClaudeToken() ? "valid" : "expired";
      case "codex": {
        if (readCodexTokens()) return "valid";
        // Also accept API-key auth stored in ~/.codex/auth.json
        if (jsonHasKey(path.join(os.homedir(), ".codex", "auth.json"), "OPENAI_API_KEY")) return "valid";
        if (process.env.OPENAI_API_KEY) return "valid";
        return "expired";
      }
      case "gemini":
        return readGeminiCredsFromKeychain() ||
          fileExistsNonEmpty(path.join(os.homedir(), ".gemini", "oauth_creds.json"))
          ? "valid"
          : "expired";
      default:
        return "unknown";
