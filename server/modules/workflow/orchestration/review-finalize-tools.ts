...
`,
    ...
    if (remainingSubtaskCount > 0) {
      ...
      // Check if only video pipeline phase subtasks remain pending — resume execution instead of blocking
      if (currentTask.workflow_pack_key === "video_preprod" && nonRenderRemaining > 0) {
        const pendingPipelinePhases = db
          .prepare("SELECT id FROM subtasks WHERE task_id = ? AND status = 'pending' AND id LIKE ?")
          .all(taskId, `${taskId}_phase_%`) as Array<{ id: string }>;
        const nonPipelineRemaining = remainingSubtaskCount - pendingRender.length - pendingPipelinePhases.length;

        if (nonPipelineRemaining === 0 && pendingPipelinePhases.length > 0) {
          db.prepare("UPDATE tasks SET status = 'in_progress', updated_at = ? WHERE id = ?").run(nowMs(), taskId);
          broadcast("task_update", db.prepare("SELECT * FROM tasks WHERE id = ?").get(taskId));
          appendTaskLog(
            taskId,
            "system",
            `Review approved: ${pendingPipelinePhases.length} pipeline phase(s) pending — resuming for next phase`,
          );
          // Self-call /run to trigger next phase routing (same pattern as run-complete-handler.ts)
          setTimeout(() => {
            try {
              const port = (deps.app?.get?.("port") as number) || 8790;
              ...
              log.warn({ err, taskId, operation: "trigger_task_rerun" }, "best-effort task re-run trigger failed");
            }
          }, 1500);
          return;
        }
  ...
  `,
    ...
    if (currentTask.workflow_pack_key === "video_preprod" && !currentTask.source_task_id) {
      ...
      for (const candidate of candidatePaths) {
        ...
        try {
          ...
          log.debug(
            { err, taskId, operation: "stat_video_artifact", path: candidate },
            "cannot stat video artifact candidate",
          );
        }
      }
        ...
        for (const root of searchRoots) {
          ...
          if (discovered) {
            try {
              ...
              log.debug(
                { err, taskId, operation: "stat_discovered_artifact", path: discovered },
                "cannot stat discovered artifact",
              );
            }
          }
        }
      ...
      try {
        ...
        log.debug({ err, taskId, operation: "query_delegated_render_tasks" }, "cannot query delegated render task IDs");
      }
      try {
        const childRows = db
          .prepare(
        ...
        log.debug(
          { err, taskId, operation: "query_child_task_rows" },
          "cannot query child task rows for remotion evidence",
        );
      }
    ...
    const finalizeApprovedReview = () => {
      ...
      // Video pipeline tasks use plain directories (not git worktrees) — skip merge/cleanup
      const isVideoTask = currentTask.workflow_pack_key === "video_preprod";
      const wtInfo = taskWorktrees.get(taskId);
      let mergeNote = "";
      if (wtInfo && !isVideoTask) {
