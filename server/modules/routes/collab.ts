...
export function registerRoutesPartB(ctx: RuntimeContext): RouteCollabExports {
  ...
  function buildMessengerReportSummary(agent: AgentRow, content: string): string {
    ...
    const detailHint = hasKorean
      ? "상세 내용은 OctoOffice 채팅창에서 확인하세요."
      : "See OctoOffice chat for full details.";
  ...
  function sendAgentMessage(
    ...
    `,
      ...
      if (persistedTaskId && isForeignKeyError(err)) {
        ...
        `,
          ...
          log.warn({ err: fallbackErr }, "drop message after FK fallback failure");
          return;
        }
      } else {
        log.warn({ err }, "drop message due to insert failure");
        return;
      }
    }
    ...
    if (shouldRelayTaskBroadcastToMessenger(messageType, receiverType, persistedTaskId)) {
      void relayTaskBroadcastToAssignedMessengerSessions(persistedTaskId, agent, messageType, content).catch((err) => {
        log.warn({ err, taskId: persistedTaskId, messageType }, "failed to relay task broadcast to messenger");
      });
    }
  }
  ...
  function detectMentions(message: string): { deptIds: string[]; agentIds: string[] } {
    ...
    // Match @department-name patterns (both with and without Korean team suffix)
  ...
  function findBestSubordinate(
    ...
    // If candidateAgentIds provided, select only from that list (manual mode, department-locked)
    ...
    // Default logic: select from entire department
    const agents = db
      .prepare(
  ...
  function getDeptRoleConstraint(deptId: string, deptName: string): string {
    const constraints: Record<string, string> = {
      research: `IMPORTANT ROLE CONSTRAINT: You belong to ${deptName} (Research). Focus ONLY on investigation, codebase analysis, bug reproduction, root cause analysis, and context building. Do NOT write production code or run test suites. If implementation is needed, document findings and hand off to the development team.`,
      planning: `IMPORTANT ROLE CONSTRAINT: You belong to ${deptName} (Planning). Focus ONLY on planning, requirements analysis, architecture design, task breakdowns, and acceptance criteria. Do NOT write production code, create design assets, or run tests. If coding/design is needed, describe requirements and specifications instead.`,
      dev: `IMPORTANT ROLE CONSTRAINT: You belong to ${deptName} (Development). Focus ONLY on coding, implementation, and technical execution according to the plan. Do NOT perform code review, write test-only changes, or make architecture decisions outside the approved plan.`,
      testing: `IMPORTANT ROLE CONSTRAINT: You belong to ${deptName} (Testing). Focus ONLY on writing tests, running test suites, verifying correctness, and documenting test results. Do NOT write production code beyond minimal test fixtures or perform code review.`,
      review: `IMPORTANT ROLE CONSTRAINT: You belong to ${deptName} (Review). Focus ONLY on code review, security auditing, plan compliance verification, and quality sign-off. Do NOT write production code or tests. If issues are found, document them clearly for the development team to fix.`,
      ...
      browser: `IMPORTANT ROLE CONSTRAINT: You belong to ${deptName} (Browser Testing). Focus ONLY on browser automation, end-to-end flows, screenshot testing, form validation, and web data extraction. Do NOT implement unrelated backend features unless required to unblock browser tests.`,
    };
    return (
      constraints[deptId] ||
