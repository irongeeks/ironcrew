...
import { createCrossAgentTaskRouter } from "../../workflow/orchestration/cross-agent-task-routing.ts";
...
export function createSubtaskDelegationBatch(deps: BatchDeps) {
  ...
  function delegateSubtaskBatch(
    ...
    const routingPlan = taskRouter.planRouting({
      parentTask,
      subtasks: subtasks.map((st) => ({
        id: st.id,
        title: st.title,
        description: st.description ?? null,
        target_department_id: st.target_department_id ?? null,
      })),
      targetDepartmentId: targetDeptId,
      coordinator: crossCoordinator,
      executorFallback: crossSub ?? manualPoolFallback ?? crossCoordinator,
      originLeaderId: originLeader?.id ?? null,
      candidateAgentIds: projectCandidateAgentIds,
    });
    ...
    broadcast("cross_dept_delivery", {
      ...
      chain_id: routingPlan.chain.chain_id,
      chain_step_index: 0,
      chain_step_total: routingPlan.chain.steps.length,
      baton_label: "Intake",
    ...
    setTimeout(() => {
      ...
      const plannedExecutor = routingPlan.registry.find(
        (row) => row.agent_id === routingPlan.selected.executor_agent_id,
      );
      const execAgent =
        (db
          .prepare("SELECT * FROM agents WHERE id = ?")
          .get(routingPlan.selected.executor_agent_id) as AgentRow | null) ??
        (plannedExecutor
          ? (db.prepare("SELECT * FROM agents WHERE id = ?").get(plannedExecutor.agent_id) as AgentRow | null)
          : null) ??
        crossSub ??
        manualPoolFallback ??
        crossCoordinator ??
      ...
      db.prepare(
        `
    INSERT INTO tasks (id, title, description, department_id, project_id, status, priority, task_type, workflow_pack_key, workflow_meta_json, project_path, source_task_id, created_at, updated_at)
  ...
  `,
        ...
        JSON.stringify({ cross_agent_routing: routingPlan }),
      ...
      taskRouter.updateChainStep(
        delegatedTaskId,
        "origin_to_coordinator",
        "completed",
        "Cross-department coordinator acknowledged the intake.",
      );
      if (crossCoordinator.id !== execAgent.id) {
        taskRouter.updateChainStep(
          delegatedTaskId,
          "coordinator_to_executor",
          "completed",
          "Coordinator routed task to specialized executor.",
        );
        broadcast("cross_dept_delivery", {
          from_agent_id: crossCoordinator.id,
          to_agent_id: execAgent.id,
          task_title: delegatedTitle,
          chain_id: routingPlan.chain.chain_id,
          chain_step_index: 1,
          chain_step_total: routingPlan.chain.steps.length,
          baton_label: "Execute",
      ...
      const failDelegatedLaunch = (error: unknown, stage: string) => {
        ...
        taskRouter.markRollback(delegatedTaskId, `Delegated launch failed (${stage}): ${message}`);
      ...
      if (
        ["claude", "codex", "gemini", "opencode", "copilot", "antigravity", "api", "openclaw"].includes(execProvider)
      ) {
        ...
        try {
          ...
          if (!projPath) {
            failDelegatedLaunch(
              new Error("project_path_required: no project path is set for the parent task"),
          ...
          if (execProvider === "api") {
            ...
            try {
              const delegateProfile = execProvider === "openclaw" ? execAgent.cli_profile || undefined : undefined;
              child = spawnCliAgent(
                delegatedTaskId,
                execProvider,
                sessionPrompt,
