...
export class BudgetEngine {
  ...
  spentFor(budget: BudgetRow, now = Date.now()): number {
    ...
    const row = this.db
      .prepare(`SELECT COALESCE(SUM(cost_micros), 0) AS total FROM ic_cost_events WHERE ${clauses.join(" AND ")}`)
