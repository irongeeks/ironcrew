/**
 * Iron Command OS — notification repository (the decision inbox's feed).
 *
...
export class NotificationStore {
  ...
  create(input: CreateNotificationInput): NotificationRow {
    ...
    this.db
      .prepare(
        `INSERT INTO ic_notifications (id, company_id, kind, severity, title, body, task_id, approval_id)
         VALUES (?,?,?,?,?,?,?,?)`,
      )
      .run(
        id,
  ...
  get(id: string): NotificationRow | null {
    return oneRow<NotificationRow>(this.db.prepare("SELECT * FROM ic_notifications WHERE id = ?"), id);
  }
  ...
  list(companyId: string, opts: { unreadOnly?: boolean; limit?: number } = {}): NotificationRow[] {
    ...
    return allRows<NotificationRow>(
      this.db.prepare(
        `SELECT * FROM ic_notifications WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC, rowid DESC LIMIT ?`,
      ),
      ...params,
    );
  }
  ...
  countUnread(companyId: string): number {
    const row = oneRow<{ n: number }>(
      this.db.prepare("SELECT COUNT(*) AS n FROM ic_notifications WHERE company_id = ? AND read_at IS NULL"),
      companyId,
    );
    return row?.n ?? 0;
  }
  ...
  markRead(id: string, now = Date.now()): NotificationRow | null {
    this.db.prepare("UPDATE ic_notifications SET read_at = COALESCE(read_at, ?) WHERE id = ?").run(now, id);
    return this.get(id);
  }
  ...
  markReadByApproval(companyId: string, approvalId: string, now = Date.now()): void {
    this.db
      .prepare("UPDATE ic_notifications SET read_at = COALESCE(read_at, ?) WHERE company_id = ? AND approval_id = ?")
