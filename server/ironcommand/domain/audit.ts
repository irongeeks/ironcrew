/**
 * Iron Command OS — append-only, hash-chained audit log.
 *
...
export function appendAuditEvent(db: DatabaseSync, input: AuditInput): AuditEvent {
  const prev = db
    .prepare("SELECT seq, entry_hash FROM ic_audit_events WHERE company_id = ? ORDER BY seq DESC LIMIT 1")
