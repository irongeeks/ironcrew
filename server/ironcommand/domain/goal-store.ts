/**
 * Iron Command OS — goal repository.
 *
...
export class GoalStore {
  ...
  create(input: CreateGoalInput): GoalRow {
    ...
    this.db
      .prepare(
        `INSERT INTO ic_goals (id, company_id, parent_id, title, description, status)
         VALUES (?,?,?,?,?,?)`,
      )
  ...
  get(id: string): GoalRow | null {
    return oneRow<GoalRow>(this.db.prepare("SELECT * FROM ic_goals WHERE id = ?"), id);
  }
  ...
  list(companyId: string, opts: { status?: GoalStatus; parentId?: string | null } = {}): GoalRow[] {
    ...
    return allRows<GoalRow>(
      this.db.prepare(`SELECT * FROM ic_goals WHERE ${clauses.join(" AND ")} ORDER BY created_at ASC`),
      ...params,
    );
  }
  ...
  children(goalId: string): GoalRow[] {
    return allRows<GoalRow>(
      this.db.prepare("SELECT * FROM ic_goals WHERE parent_id = ? ORDER BY created_at ASC"),
      goalId,
    );
  }
  ...
  update(
    ...
    if (!goal) return null;
    this.db
      .prepare("UPDATE ic_goals SET title = COALESCE(?, title), description = COALESCE(?, description) WHERE id = ?")
  ...
  reparent(
    ...
    this.db.prepare("UPDATE ic_goals SET parent_id = ? WHERE id = ?").run(newParentId, goalId);
  ...
  setStatus(
    ...
    this.db.prepare("UPDATE ic_goals SET status = ? WHERE id = ?").run(status, goalId);
