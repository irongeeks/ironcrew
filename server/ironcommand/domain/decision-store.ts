...
export class DecisionStore {
  ...
  create(input: CreateDecisionInput): DecisionRow {
    ...
    this.db
      .prepare(
        `INSERT INTO ic_decisions (id, company_id, project_id, task_id, title, context, decision, rationale, decided_by)
         VALUES (?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        id,
  ...
  get(id: string): DecisionRow | null {
    return oneRow<DecisionRow>(this.db.prepare("SELECT * FROM ic_decisions WHERE id = ?"), id);
  }
  ...
  list(companyId: string, opts: { projectId?: string; taskId?: string; limit?: number } = {}): DecisionRow[] {
    ...
    return allRows<DecisionRow>(
      this.db.prepare(
        `SELECT * FROM ic_decisions WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC, rowid DESC LIMIT ?`,
      ),
      ...params,
    );
  }
