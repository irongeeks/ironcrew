...
async function fetchDiscordJson(token: string, apiPath: string): Promise<unknown> {
  ...
  try {
    ...
    throw new Error(`discord api returned invalid JSON (${response.status}): ${bodyText.slice(0, 200)}`);
  }
}
...
function readPersistedMessengerChannels(): PersistedMessengerChannels | null {
  ...
  try {
    ...
    log.warn({ err }, "failed to load messenger channels settings");
    return null;
  } finally {
    try {
      db?.close();
...
function queueWake(params: { key: string; text: string; debounceMs?: number }) {
  ...
  void sendMessengerWake(params.text).catch((err) => {
    log.warn({ err, key: params.key }, "messenger notification failed");
  });
}
...
/**
 * Notify configured messenger channels that a pipeline phase needs user approval.
 * Includes a decision token so the user can approve by replying.
 */
export function notifyPhaseApprovalNeeded(taskId: string, taskTitle: string, phaseId: string, summary?: string): void {
  const lines = [`\u23F8 [Approval Needed] ${taskTitle}`, `Phase: ${phaseId}`];
  if (summary) {
    lines.push(`Summary: ${summary.slice(0, 200)}`);
  }
  lines.push(
    `Reply "approve ${phaseId}" to approve (or "approve ${taskId}/${phaseId}" if multiple tasks are waiting).`,
  );

  queueWake({
    key: `phase-approval:${taskId}:${phaseId}`,
    text: lines.join("\n"),
    debounceMs: 10_000,
  });
}
