...
describe("create / read", () => {
  ...
  it("writes an audit entry on creation", () => {
    const t = readyTask();
    const rows = db.prepare("SELECT * FROM crew_audit_events WHERE task_id = ? AND action = 'task.created'").all(t.id);
...
describe("atomic claiming (the double-work guarantee)", () => {
  ...
  it("audits the claim", () => {
    ...
    const rows = db.prepare("SELECT * FROM crew_audit_events WHERE action='task.claimed'").all();
    expect(rows).toHaveLength(1);
  });
});
...
describe("dependencies", () => {
  ...
  it("audits an added dependency", () => {
    ...
    store.addDependency(companyId, a.id, b.id);
    const rows = db
      .prepare("SELECT action FROM crew_audit_events WHERE entity_id = ? AND action = 'task.dependency_added'")
      .all(a.id);
    expect(rows).toHaveLength(1);
  });
  ...
  it("audits a removed dependency", () => {
    ...
    const rows = db
      .prepare("SELECT action FROM crew_audit_events WHERE entity_id = ? AND action = 'task.dependency_removed'")
      .all(a.id);
    expect(rows).toHaveLength(1);
  });
});
...
describe("audit chain integrity", () => {
  ...
  it("detects a tampered historical entry", () => {
    ...
    db.prepare("UPDATE crew_audit_events SET action = 'task.forged' WHERE seq = 2 AND company_id = ?").run(companyId);
  ...
  it("detects a deleted entry as a sequence gap", () => {
    ...
    store.transition(t.id, "assigned");
    store.transition(t.id, "running");
    db.prepare("DELETE FROM crew_audit_events WHERE seq = 2 AND company_id = ?").run(companyId);
