/**
 * IronCrew — sandbox grant store.
 *
 * A grant is what makes `elevated` CLI permission mode reachable
 * (server/ironcrew/policy/runtime-permissions.ts). This module is the
...
export class SandboxGrantStore {
  ...
  mintFromApproval(input: {
    ...
    this.db
      .prepare(
        `INSERT INTO crew_sandbox_grants
  ...
  get(id: string): SandboxGrantRow | null {
    return (
      (this.db.prepare("SELECT * FROM crew_sandbox_grants WHERE id = ?").get(id) as SandboxGrantRow | undefined) ?? null
    );
  }
  ...
  revoke(id: string, actorId: string, reason = ""): SandboxGrantRow | null {
    ...
    this.db.prepare("UPDATE crew_sandbox_grants SET revoked_at = ? WHERE id = ?").run(Date.now(), id);
