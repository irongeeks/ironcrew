...
export class TaskStore {
  ...
  get(id: string): TaskRow | null {
    return (this.db.prepare("SELECT * FROM crew_tasks WHERE id = ?").get(id) as TaskRow | undefined) ?? null;
  }
  ...
  list(companyId: string, opts: { status?: TaskStatus; projectId?: string; limit?: number } = {}): TaskRow[] {
    ...
    return allRows<TaskRow>(
      this.db.prepare(`SELECT * FROM crew_tasks WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC LIMIT ?`),
      ...params,
    );
  }
  ...
  renewLock(taskId: string, runId: string, opts: { lockTtlMs?: number; now?: number } = {}): boolean {
    ...
    const res = this.db
      .prepare(
        `UPDATE crew_tasks SET lock_expires_at = ?, updated_at = ?
  ...
  addDependency(
    ...
    this.db
      .prepare(
        `INSERT OR IGNORE INTO crew_task_dependencies (id, company_id, task_id, depends_on_id)
         VALUES (?,?,?,?)`,
      )
  ...
  removeDependency(
    ...
    const res = this.db
      .prepare("DELETE FROM crew_task_dependencies WHERE company_id = ? AND task_id = ? AND depends_on_id = ?")
  ...
  private wouldCreateCycle(taskId: string, dependsOnId: string): boolean {
    ...
    while (stack.length) {
      ...
      seen.add(current);
      const rows = this.db
        .prepare("SELECT depends_on_id FROM crew_task_dependencies WHERE task_id = ?")
  ...
  blockers(taskId: string): TaskRow[] {
    return this.db
      .prepare(
        `SELECT t.* FROM crew_tasks t
           JOIN crew_task_dependencies d ON d.depends_on_id = t.id
          WHERE d.task_id = ?`,
      )
  ...
  blocking(taskId: string): TaskRow[] {
    return this.db
      .prepare(
        `SELECT t.* FROM crew_tasks t
           JOIN crew_task_dependencies d ON d.task_id = t.id
