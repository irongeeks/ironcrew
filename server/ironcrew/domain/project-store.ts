/**
 * IronCrew — project and milestone repository.
 *
 ...
 * so they live in crew_milestones rather than crew_tasks (see the migration's
 * own comment for why).
 */
...
export class ProjectStore {
  ...
  create(input: CreateProjectInput): ProjectRow {
    if (input.goalId) {
      const goal = oneRow<{ company_id: string }>(
        this.db.prepare("SELECT company_id FROM crew_goals WHERE id = ?"),
        input.goalId,
      );
    ...
    for (let attempt = 0; attempt < 20; attempt++) {
      try {
        this.db
          .prepare(
            `INSERT INTO crew_projects (id, company_id, goal_id, key, title, summary, status, owner_agent_id, workspace_path)
             VALUES (?,?,?,?,?,?,?,?,?)`,
          )
          .run(
            id,
  ...
  get(id: string): ProjectRow | null {
    return oneRow<ProjectRow>(this.db.prepare("SELECT * FROM crew_projects WHERE id = ?"), id);
  }

  getByKey(companyId: string, key: string): ProjectRow | null {
    return oneRow<ProjectRow>(
      this.db.prepare("SELECT * FROM crew_projects WHERE company_id = ? AND key = ?"),
      companyId,
      key,
    );
  }
  ...
  list(companyId: string, opts: { status?: ProjectStatus; goalId?: string } = {}): ProjectRow[] {
    ...
    return allRows<ProjectRow>(
      this.db.prepare(`SELECT * FROM crew_projects WHERE ${clauses.join(" AND ")} ORDER BY created_at DESC`),
      ...params,
    );
  }
  ...
  setStatus(
    ...
    this.db
      .prepare("UPDATE crew_projects SET status = ?, updated_at = ? WHERE id = ?")
  ...
  addMilestone(input: CreateMilestoneInput): MilestoneRow {
    ...
    this.db
      .prepare(
        `INSERT INTO crew_milestones (id, company_id, project_id, title, description, due_at, sort_order)
         VALUES (?,?,?,?,?,?,?)`,
      )
      .run(
        id,
  ...
  getMilestone(id: string): MilestoneRow | null {
    return oneRow<MilestoneRow>(this.db.prepare("SELECT * FROM crew_milestones WHERE id = ?"), id);
  }
  ...
  listMilestones(projectId: string, opts: { status?: MilestoneStatus } = {}): MilestoneRow[] {
    if (opts.status) {
      return allRows<MilestoneRow>(
        this.db.prepare(
          "SELECT * FROM crew_milestones WHERE project_id = ? AND status = ? ORDER BY sort_order ASC, due_at ASC",
        ),
        projectId,
        opts.status,
      );
    ...
    return allRows<MilestoneRow>(
      this.db.prepare("SELECT * FROM crew_milestones WHERE project_id = ? ORDER BY sort_order ASC, due_at ASC"),
      projectId,
    );
  }
