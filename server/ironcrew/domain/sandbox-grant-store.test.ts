...
describe("mintFromApproval — the only path to a grant", () => {
  ...
  it("audits the mint", () => {
    ...
    const rows = db.prepare("SELECT action FROM crew_audit_events WHERE action='sandbox_grant.minted'").all();
...
describe("revoke", () => {
  ...
  it("audits the revocation", () => {
    ...
    const grant = store.mintFromApproval({ approval, providers: ["claude"], requestedDurationMs: 60_000 });
    store.revoke(grant.id, "owner-1", "no longer needed");
    const rows = db.prepare("SELECT action FROM crew_audit_events WHERE action='sandbox_grant.revoked'").all();
    expect(rows).toHaveLength(1);
  });
});
