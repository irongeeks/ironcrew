...
describe("create / read", () => {
  ...
  it("audits creation", () => {
    ...
    const rows = db.prepare("SELECT action FROM crew_audit_events WHERE entity_id = ?").all(g.id) as Array<{
      action: string;
    }>;
...
describe("status transitions", () => {
  ...
  it("audits every status change", () => {
    ...
    store.setStatus(g.id, "achieved");
    const rows = db
      .prepare("SELECT action FROM crew_audit_events WHERE entity_id = ? AND action = 'goal.status_changed'")
      .all(g.id);
