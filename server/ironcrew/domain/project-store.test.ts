...
describe("create / read", () => {
  ...
  it("audits creation", () => {
    ...
    const rows = db.prepare("SELECT action FROM crew_audit_events WHERE entity_id = ?").all(p.id) as Array<{
      action: string;
    }>;
...
describe("status transitions", () => {
  ...
  it("audits every status change", () => {
    ...
    const rows = db
      .prepare("SELECT action FROM crew_audit_events WHERE entity_id = ? AND action = 'project.status_changed'")
      .all(p.id);
