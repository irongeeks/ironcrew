/**
 * IronCrew — run + run-event persistence.
 *
...
export class RunStore {
  ...
  heartbeat(runId: string, now = Date.now()): void {
    this.db.prepare("UPDATE crew_runs SET heartbeat_at = ? WHERE id = ?").run(now, runId);
  }
  ...
  appendEvent(input: {
    ...
    const seqRow = this.db
      .prepare("UPDATE crew_runs SET next_event_seq = next_event_seq + 1 WHERE id = ? RETURNING next_event_seq")
  ...
  listEvents(runId: string, opts: { afterSeq?: number; limit?: number } = {}): RunEvent[] {
    ...
    const rows = this.db
      .prepare(`SELECT * FROM crew_run_events WHERE run_id = ? AND seq > ? ORDER BY seq ASC LIMIT ?`)
